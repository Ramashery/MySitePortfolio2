...
// --- SEO & DATA FUNCTIONS ---
// --- ИЗМЕНЕННЫЙ БЛОК: Улучшенная функция рендеринга SEO-тегов ---
function renderSeoTags(data) {
    // 1. Очищаем старые теги, которые мы будем заменять.
    ...
    // 4. Устанавливаем OG:Image: приоритет у специального поля, затем fallback на первое изображение.
    const ogImage = data.ogImage || data.media?.find(url => !/youtube|vimeo/.test(url)) || '';
    if (ogImage) createMeta('property', 'og:image', ogImage);
    
    // 5. ИСПРАВЛЕННЫЙ БЛОК: Создаем ПРАВИЛЬНУЮ каноническую ссылку.
    const canonical = document.createElement('link');
    canonical.rel = 'canonical';
    const canonicalBaseUrl = 'https://digital-craft-tbilisi.netlify.app'; // Ваш основной домен
    
    // Получаем текущий путь из адресной строки
    let cleanPath = window.location.pathname;
    
    // Если путь не является корневым ('/') и заканчивается на слэш, убираем его
    if (cleanPath.length > 1 && cleanPath.endsWith('/')) {
      cleanPath = cleanPath.slice(0, -1);
    }
    
    // Устанавливаем канонический URL, который всегда будет без слэша на конце (кроме главной)
    canonical.href = canonicalBaseUrl + cleanPath;
    document.head.appendChild(canonical);
    
    // 6. Вставляем JSON-LD для Schema.org. Googlebot это увидит.
...
// --- ИЗМЕНЕННЫЙ БЛОК: Добавлены поля для OG-тегов в форму админ-панели ---
...
function renderAdminSection(key) {
    ...
    const listsHTML = langOrder.map(lang => {
        ...
        const itemsListHTML = groupedItems[lang]
            ...
            .map(item => `
                <li class="admin-list-item" data-id="${item.id}" data-key="${key}">
                    ${item.title || 'No Title'}
        ...
        return `
            <div class="admin-lang-group">
                <h4>${langNames[lang]} (${lang})</h4>
                <ul class="admin-item-list">${itemsListHTML}</ul>
            </div>
        `;
    }).join('');
...
async function handleAdminActions(e) { const target = e.target; const action = target.dataset.action; if (!action) return; const itemEl = target.closest('.admin-item'); const setEditingState = (el, isEditing) => { el.classList.toggle('is-editing', isEditing); el.querySelectorAll('input, textarea, select').forEach(input => input.disabled = !isEditing); }; try { if (action === 'edit-home') { setEditingState(itemEl, true); return; } if (action === 'save-home') { setEditingState(itemEl, false); let schemaValue = itemEl.querySelector('#home-schemaJsonLd').value; try { schemaValue = JSON.parse(schemaValue); } catch(err) { console.error("Invalid JSON in home schema", err); alert("Error: Invalid JSON in Schema field."); return; } const updatedData = { h1: itemEl.querySelector('#home-h1').value, subtitle: itemEl.querySelector('#home-subtitle').value, lang: itemEl.querySelector('#home-lang').value, seoTitle: itemEl.querySelector('#home-seoTitle').value, metaDescription: itemEl.querySelector('#home-metaDescription').value, schemaJsonLd: schemaValue, ogTitle: itemEl.querySelector('#home-ogTitle').value, ogDescription: itemEl.querySelector('#home-ogDescription').value, ogImage: itemEl.querySelector('#home-ogImage').value, backgroundHtml: itemEl.querySelector('#home-backgroundHtml').value, }; await db.collection('home').doc('content').update(updatedData); await loadData().then(freshData => { siteData = freshData; routeAndRender(); }); alert('Home page updated!'); return; } const key = target.dataset.key || itemEl.dataset.key; const id = itemEl ? itemEl.dataset.id : null; switch(action) { case 'edit': setEditingState(itemEl, true); break; 
    ...
    case 'save': { 
        ...
        try { 
            ...
            console.error("Invalid JSON in item schema", err); 
            alert("Error: Invalid JSON in Schema field."); 
            return; 
        } 
        const updatedData = { 
