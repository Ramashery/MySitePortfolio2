...
function renderSeoTags(data) {
    ...
    const canonicalBaseUrl = 'https://' + window.location.hostname;
...
function updateMetaTags(itemData = {}) { 
    ...
    if (dataToRender) renderSeoTags(dataToRender);
}
...
function handleNavigation(e) { const link = e.target.closest('a'); if (!link || link.target === '_blank' || link.protocol !== window.location.protocol || link.host !== window.location.host || e.metaKey || e.ctrlKey || e.shiftKey) { return; } const linkUrl = new URL(link.href); const isAnchorLink = !!linkUrl.hash; if (isAnchorLink) { e.preventDefault(); const targetPath = linkUrl.pathname.replace(/\/$/, ""); const currentPath = window.location.pathname.replace(/\/$/, ""); const targetElementId = linkUrl.hash.substring(1); const scrollToElement = () => { const element = document.getElementById(targetElementId); if (element) { setTimeout(() => { element.scrollIntoView({ behavior: 'smooth' }); }, 100); } }; if (targetPath === currentPath) { scrollToElement(); } else { window.history.pushState({}, '', link.href); routeAndRender(); } return; } if (linkUrl.href === window.location.href) { e.preventDefault(); return; } e.preventDefault(); window.history.pushState({}, '', linkUrl.pathname + linkUrl.search + linkUrl.hash); routeAndRender(); window.scrollTo(0, 0); };
function initStaticEventListeners() { document.body.addEventListener('click', handleNavigation); window.addEventListener('popstate', routeAndRender); let resizeTimer; window.addEventListener('resize', () => { clearTimeout(resizeTimer); resizeTimer = setTimeout(() => { document.querySelectorAll('.cross-fade-slider').forEach(slider => { const activeSlide = slider.querySelector('.item-card.active'); if (activeSlide) { slider.style.height = `${activeSlide.offsetHeight}px`; } }); }, 200); }); const menuToggle = document.querySelector('.menu-toggle'); const navOverlay = document.querySelector('.nav-overlay'); menuToggle.addEventListener('click', (e) => { e.stopPropagation(); document.body.classList.toggle('nav-is-open'); menuToggle.classList.toggle('is-active'); navOverlay.classList.toggle('is-active'); }); navOverlay.addEventListener('click', (e) => { const link = e.target.closest('a'); if (link || e.target === navOverlay) { document.body.classList.remove('nav-is-open'); menuToggle.classList.remove('is-active'); navOverlay.classList.remove('is-active'); } }); const adminCloseBtn = document.getElementById('admin-close-btn'); if (adminCloseBtn) adminCloseBtn.addEventListener('click', (e) => { e.stopPropagation(); document.getElementById('admin-panel').classList.remove('active');}); const adminContent = document.querySelector('.admin-content'); if (adminContent) adminContent.addEventListener('click', e => { e.stopPropagation(); const listItem = e.target.closest('.admin-list-item'); if (listItem) { const id = listItem.dataset.id; const key = listItem.dataset.key; const itemData = siteData[key]?.find(i => i.id === id); if (itemData) { const tabContent = listItem.closest('.tab-content'); tabContent.querySelectorAll('.admin-list-item').forEach(el => el.classList.remove('selected')); listItem.classList.add('selected'); const editorContainer = tabContent.querySelector('.admin-item-editor-container'); editorContainer.innerHTML = generateAdminItemFormHTML(itemData, key); } return; } handleAdminActions(e); }); const adminTabs = document.querySelector('.admin-tabs'); if (adminTabs) adminTabs.addEventListener('click', e => { e.stopPropagation(); if (e.target.matches('.admin-tab')) { document.querySelectorAll('.admin-tab, .tab-content').forEach(el => el.classList.remove('active')); e.target.classList.add('active'); document.querySelector(`.tab-content[data-tab-content="${e.target.dataset.tab}"]`).classList.add('active'); } }); const footer = document.getElementById('site-footer'); if (footer) { footer.innerHTML = `© 2025 Digital Craft. All rights reserved.`; footer.addEventListener('click', handleAdminLogin); } }
function applyCustomBackground(item = null) { const iframe = document.getElementById('custom-background-iframe'); if(!iframe) return; const customCode = item?.backgroundHtml || siteData.home?.backgroundHtml || ''; if (customCode && customCode.trim() !== "") { iframe.style.display = 'block'; iframe.srcdoc = customCode; } else { iframe.style.display = 'none'; iframe.srcdoc = ''; } }

// === ИЗМЕНЕНИЕ: ГЛАВНАЯ ФУНКЦИЯ-ИНИЦИАЛИЗАТОР ===
async function main() {
  initStaticEventListeners();
  try {
    const freshSiteData = await loadData();
    ...
    // Показываем ошибку на экране, если что-то пошло не так
    document.body.innerHTML = `
      <div style="color: white; background: #1a1a1a; padding: 40px; font-family: monospace; line-height: 1.6;">
        <h1>Application Error</h1>
        <p>A critical error occurred. Please check the console or contact support.</p>
        <pre style="background: #000; padding: 20px; border-radius: 8px; white-space: pre-wrap; word-wrap: break-word;">${error.stack}</pre>
      </div>
    `;
  } finally {
    // Эта команда выполнится всегда, даже если была ошибка, и уберет прелоадер
    const loader = document.getElementById('loader');
    if(loader) loader.classList.add('hidden');
  }
}

// === ИЗМЕНЕНИЕ: ЗАПУСКАЕМ ПРИЛОЖЕНИЕ НАПРЯМУЮ ===
// Вместо DOMContentLoaded, который больше не нужен для модулей.
main();