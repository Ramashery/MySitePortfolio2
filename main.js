...
function updateMetaTags(itemData = {}) { const dataToRender = (itemData && Object.keys(itemData).length && itemData.seoTitle) ? itemData : siteData.home; renderSeoTags(dataToRender); }
...
function renderSection(key, title, items) {
    ...
    // --- НАЧАЛО: НОВЫЙ КОД ДЛЯ ДЕСКТОПНЫХ СЛАЙДЕРОВ ---
    const desktopSlidersHTML = langOrder.map(lang => {
        const langItems = itemsByLang[lang];
        if (!langItems || langItems.length === 0) return '';

        // Разбиваем карточки на группы по 3
        const chunks = [];
        for (let i = 0; i < langItems.length; i += 3) {
            chunks.push(langItems.slice(i, i + 3));
        }

        const slidesHTML = chunks.map((chunk, chunkIndex) => {
            const cardsHTML = chunk.map(item => {
                const langPrefix = item.lang ? `/${item.lang}` : '';
                const itemUrl = `${langPrefix}/${key}/${item.urlSlug}`;
                return `<a href="${itemUrl}" class="item-card"><div class="item-card__image" style="background-image: url('${(item.media || []).find(url => !/youtube|vimeo/.test(url)) || ''}')"></div><div class="item-card__content"><h3>${item.title}</h3><div class="card-subtitle">${item.subtitle}</div><p>${item.description}</p></div></a>`;
            }).join('');
            return `<div class="desktop-slide ${chunkIndex === 0 ? 'active' : ''}">${cardsHTML}</div>`;
        }).join('');

        const dotsHTML = chunks.length > 1 
            ? `<div class="desktop-slider-dots">${chunks.map((_, i) => `<span class="desktop-slider-dot ${i === 0 ? 'active' : ''}" data-index="${i}"></span>`).join('')}</div>`
            : '';

        return `
            <div class="desktop-slider-group" data-lang="${lang}">
                <h3 class="desktop-slider-title">${langNames[lang]}</h3>
                <div class="desktop-slider-content">${slidesHTML}</div>
                ${dotsHTML}
            </div>
        `;
    }).join('');
    const desktopContainer = `<div class="desktop-sliders-container">${desktopSlidersHTML}</div>`;
    // --- КОНЕЦ: НОВЫЙ КОД ДЛЯ ДЕСКТОПНЫХ СЛАЙДЕРОВ ---


    // --- СУЩЕСТВУЮЩИЙ КОД ДЛЯ МОБИЛЬНЫХ СЛАЙДЕРОВ (НЕ ТРОГАЕМ) ---
    ...
    // Собираем финальный HTML для секции
    section.innerHTML = `<div class="animated-container"><h2>${title}</h2></div>${desktopContainer}${mobileContainer}`;
};
...
function initDesktopSliders() {
    document.querySelectorAll('.desktop-slider-group').forEach(sliderGroup => {
        const content = sliderGroup.querySelector('.desktop-slider-content');
        const slides = sliderGroup.querySelectorAll('.desktop-slide');
        const dots = sliderGroup.querySelectorAll('.desktop-slider-dot');

        if (slides.length <= 1) {
            // Если слайд только один, просто установим высоту и выйдем
            const activeSlide = slides[0];
            if(activeSlide) content.style.height = `${activeSlide.offsetHeight}px`;
            return;
        }
        ...
        const updateSliderHeight = () => {
            ...
            if (activeSlide) {
                content.style.height = `${activeSlide.offsetHeight}px`;
            }
        };
        ...
        sliderGroup.querySelector('.desktop-slider-dots')?.addEventListener('click', e => {
        ...
        // Запускаем
        setTimeout(updateSliderHeight, 100); // Начальная установка высоты
        startAutoSlide();
    });
}
