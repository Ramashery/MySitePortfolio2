// --- НАЧАЛО ОКОНЧАТЕЛЬНОЙ ВЕРСИИ main.js (v3.0 - ОТКАЗОУСТОЙЧИВАЯ) ---
...
const { basePath } = getPathDetails(); // Определяем один раз при запуске
...
async function App() {
    console.log(`App running. Initial load: ${isInitialLoad}`);
    ...
    try {
        ...
        if (match) {
            ...
            await renderDetailPage(collection, slug, lang || defaultLang);
        } else {
        ...
        console.error("CRITICAL: App render cycle failed!", error);
        render404Page({ error: true });
    } finally {
        document.getElementById('loader').classList.add('hidden');
        console.log("App render cycle finished.");
    }
}
...
async function renderHomePage() {
    ...
    if (!siteData.home.h1) {
        ...
        } catch (e) { console.error("Could not fetch home data", e); }
    }
    
    renderSeoTags(siteData.home || { seoTitle: 'Digital Craft' });
...
async function renderDetailPage(collection, slug, lang) {
    console.log(`Rendering Detail Page: ${collection}/${slug}`);
    let itemData;
    try {
        const snapshot = await db.collection(collection).get();
        const allItems = snapshot.docs.map(doc => ({ id: doc.id, ...processDocData(doc.data()) }));
        itemData = allItems.find(item => item.urlSlug === slug && item.lang === lang);
    } catch (e) {
        console.error(`Could not fetch data for ${collection}`, e);
        itemData = siteData[collection]?.find(item => item.urlSlug === slug && item.lang === lang);
    }
    ...
    if (itemData) {
        ...
        mainContentEl.innerHTML = `
            <section>
                <div class="detail-page-header"><h1 class="fade-in-up">${itemData.h1 || ''}</h1>${itemData.price ? `<div class="detail-price fade-in-up">${itemData.price}</div>` : ''}</div>
...
// =================================================================== //
// ==================== "ОЖИВЛЕНИЕ" И НАВИГАЦИЯ ======================= //
// =================================================================== //

function hydratePage() {
    console.log("Hydrating page with event listeners and loading full data...");
    initStaticEventListeners();
    renderMenu();
    renderAdminPanel();
    loadAllSiteDataInBackground();
}

async function loadAllSiteDataInBackground() {
    try {
        ...
        if (document.getElementById('services') && mainContentEl.querySelector('#services .spinner')) {
             renderAllHomeSections();
        }
        if(document.querySelector('.detail-content') && !document.getElementById('related-posts')) {
             const { pathWithoutBase } = getPathDetails();
             const detailPageRegex = /^\/(?:([a-z]{2})\/)?(services|portfolio|blog|contact)\/([a-zA-Z0-9-]+)\/?$/;
             const match = pathWithoutBase.match(detailPageRegex);
             if(match) {
        ...
        console.error("Error loading full site data in background:", error);
    }
}
...
App();
// --- КОНЕЦ ОКОНЧАТЕЛЬНОЙ ВЕРСИИ main.js (v3.0 - ОТКАЗОУСТОЙЧИВАЯ) ---