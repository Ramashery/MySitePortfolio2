// --- НАЧАЛО ОКОНЧАТЕЛЬНОЙ ВЕРСИИ main.js (v4.0 - НАДЕЖНАЯ АРХИТЕКТУРА) ---
...
let siteData = null; // null означает, что данные еще не загружены
...
async function App() {
    console.log("App Cycle Start.");
    if (!isInitialLoad) {
        document.getElementById('loader').classList.remove('hidden');
    }

    try {
        // ШАГ 1: ЗАГРУЗКА ДАННЫХ (только при первой загрузке)
        if (isInitialLoad) {
            siteData = await fetchAllSiteData();
            console.log("Initial data fetched successfully.");
            hydratePage(); // Устанавливаем обработчики событий
            isInitialLoad = false;
        }

        // ШАГ 2: РЕНДЕРИНГ СТРАНИЦЫ (используя загруженные данные)
        ...
        // ШАГ 3: ГАРАНТИРОВАННО СКРЫВАЕМ ПРЕЛОАДЕР
        document.getElementById('loader').classList.add('hidden');
        console.log("App Cycle End.");
    }
}

// =================================================================== //
// ==================== ЛОГИКА ЗАГРУЗКИ И РЕНДЕРИНГА ================== //
// =================================================================== //

async function fetchAllSiteData() {
    console.log("Fetching all site data from Firebase...");
    ...
    try {
        ...
        console.error("Failed to fetch site data from Firebase!", error);
        throw error; // Пробрасываем ошибку выше, чтобы App() ее поймал
    }
}
...
function renderDetailPage(itemData, collection) {
    console.log(`Rendering Detail Page: ${collection}/${itemData.urlSlug}`);
...
function hydratePage() {
    console.log("Hydrating page with event listeners...");
...
function handleNavigation(e) {
    ...
    App(); // Запускаем главный цикл для новой страницы
}
...
App();
// --- КОНЕЦ ОКОНЧАТЕЛЬНОЙ ВЕРСИИ main.js (v4.0 - НАДЕЖНАЯ АРХИТЕКТУРА) ---